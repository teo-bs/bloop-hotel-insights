...
-- Create unique constraint for upserts (user_id, provider, external_review_id)
CREATE UNIQUE INDEX IF NOT EXISTS idx_reviews_unique_external 
ON reviews (user_id, provider, external_review_id) 
...
ON reviews (user_id, provider, date, text) 
