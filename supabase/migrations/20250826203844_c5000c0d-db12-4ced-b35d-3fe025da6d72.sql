-- Core tables (step by step to avoid circular references)
...
ON public.hotels FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.hotel_members m WHERE m.hotel_id = id AND m.user_id = auth.uid())
);
...
CREATE POLICY "users can read hotel members for their hotels"
ON public.hotel_members FOR SELECT USING (
  user_id = auth.uid() OR EXISTS (SELECT 1 FROM public.hotel_members m WHERE m.hotel_id = hotel_members.hotel_id AND m.user_id = auth.uid())
);

CREATE POLICY "owners and managers can manage hotel members"
